<script lang="ts">
  import { Map, MapGeoJSONFeature, Marker } from "maplibre-gl";
  ...
  let searchObject: { [key: string]: MapGeoJSONFeature } = {};
  let selectedObject: { [key: string]: string } = $state({});
  ...
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  let searchControl: SearchControl = $state(null);
  ...
  onMount(() => {
    ...
    fetch(searchOption.url)
      .then((res) => res.json())
      .then((data) => {
        data.features.forEach((feature: MapGeoJSONFeature) => {
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore
...
<div class="data-container" bind:this={searchContainer}>
  <AutoComplete
    ...
    labelFunction={(properties) =>
      properties ? searchOption.format(properties) : ""}
